...
*This phase first develops every Idea Generation seed that cleared the entry threshold into a fully-specified argument — explicit step-by-step Mechanism, dimensioned Impact, and (for principled arguments) concrete grounding per the Principled Argument Blueprint in Idea Generation. Only once an argument is developed to this level can its intrinsic quality actually be judged: a seed's "rough mechanism" and "rough impact area" (the Idea Generation bar) are not yet scoreable against Claim Clarity, Mechanism, and Impact & Weighing below — scoring a seed instead of a developed argument means guessing. Scoring is applied second, to the developed argument, and filters out whatever the development process reveals to be weak: a mechanism that could not actually be built into a real causal chain, or an impact that stayed generic even under the dimensioned Impact format.*
...
### Building Out Each Argument

Every seed that cleared the Idea Generation entry threshold — relevant to the motion, sketched mechanism, identified impact area — is developed here before scoring, using the Argument Format above. This is distinct from Idea Generation: idea generation produces breadth, this produces depth on what ideation already found. Nothing is filtered out yet on the basis of intrinsic quality; development may reveal an argument is weak, but that gets confirmed by scoring next, not decided here.
...
Scoring is split into two parts to reflect the difference between intrinsic argument quality and contextual fit.

**Intrinsic Quality Score (Claim Clarity, Mechanism, Impact & Weighing)**

Each check is scored on a 1-5 scale, then multiplied by its weight. The weighted scores are summed to produce a total intrinsic quality score out of 5.
...
**Contextual Modifiers (Clash Multiplier, Bench Fit Check)**

These aren't scored on the same 1-5 scale as the three quality checks above. They are applied as adjustments after the intrinsic score is calculated.

- **Clash Multiplier**: Applied as a multiplier to the intrinsic score.
  ...
  **The Clash Multiplier measures topical centrality only — how directly the argument sits on the motion's core contested ground — never the strength or magnitude of whatever it argues against.** A rebuttal that engages the heart of a live clash point earns the 1.2 multiplier regardless of how strong the opposing argument turns out to be. If that opposing argument is weaker in the real world than its own framing suggests, that has to show up in the rebuttal's own Impact score (Impact & Weighing — see the Rebuttal Magnitude Check) instead, not in a discounted Clash Multiplier. Centrality and magnitude are different questions; conflating them either lets a low-magnitude rebuttal borrow undeserved weight from its topicality, or wrongly discounts a genuinely central clash point over an unrelated concern about its target's strength.

- **Bench Fit Check**: Applied as a binary flag.
...
Final Score = (Weighted Intrinsic Score) x (Clash Multiplier)
              filtered by Bench Fit Check
```

---

#### Weighting Options for Experimentation

The weights for the three quality checks (Claim Clarity, Mechanism, Impact & Weighing) are not fixed. Three weightings are defined for experimentation. The same set of arguments will be scored under all three weightings and ranked. The weighting whose ranking best matches expert debater judgment will be adopted.

**Mechanism-Dominant Weighting**
...
*This phase evaluates arguments based on their fit within the specific debate round — how central they are to the core clash of the motion, how comparative and non-derivative they are relative to what other teams will argue, and whether they are the right arguments for the team's bench position. No argument is ranked here in isolation; all assessments are relative to what the round actually looks like. Final Ranking outputs a ranked shortlist of 4 arguments — BP cases will typically deploy only the top 1-2, while longer formats (e.g. WSDC) can draw on more of the ranked list.*
...
**Test for Closing benches:** subtract the opening bench's material from the round entirely — does this argument's Claim still stand as a complete, distinct contribution, or does it collapse into a restatement of what opening already established? If nothing is left once the opening bench's content is removed, the argument is derivative regardless of how differently it's worded on the surface. This is the same removal test used to find an argument's load-bearing element in Development & Scoring (see Argument Format, Label) — applied here to the bench's contribution as a whole rather than a single argument's internal components.
